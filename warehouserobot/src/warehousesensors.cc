...
WarehouserobotSensors::WarehouserobotSensors(){
    ONDEBUG_INFO(std::cout << _log_prefix << "WarehouserobotSensors::WarehouserobotSensors()" << std::endl;)
}
...
void
WarehouserobotSensors::Load(ModelPtr model){
    this->model = model;
...
void
WarehouserobotSensors::Init(){

  for (int id=0; id<NUM_CAMS; id++){
     if (cam[id]){
         sub_cam[id] = cam[id]->ConnectUpdated(
             boost::bind(&WarehouserobotSensors::_on_cam, this, id));
     }else
    ...
    if (laser){
        sub_laser = laser->ConnectUpdated(
            boost::bind(&WarehouserobotSensors::_on_laser, this));
    }else
    ...
    if (bumper){
        sub_bumper = bumper->ConnectUpdated(
            boost::bind(&WarehouserobotSensors::_on_bumper, this));
    ...
    updatePose = gazebo::event::Events::ConnectWorldUpdateBegin(
                boost::bind(&WarehouserobotSensors::OnUpdate, this, _1));
}

void
WarehouserobotSensors::debugInfo(){
...
uadRotorSensors::_on_cam(int id){
...
WarehouserobotSensors::_on_laser(){
    LaserD data;
...
void
WarehouserobotSensors::_on_bumper(){

    BumperD data;

    //bumper values
    std::map<std::string, gazebo::physics::Contact> contacts;
    contacts = bumper->Contacts("warehouserobot::base::base_collision");
...
void
WarehouserobotSensors::OnUpdate(const gazebo::common::UpdateInfo & /*_info*/){
    pose = model->GetWorldPose();
}
